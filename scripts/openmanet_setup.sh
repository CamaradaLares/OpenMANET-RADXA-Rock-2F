...
case "${MODE}" in
    b)
        ...
        # Copy board-specific files overlay into the build tree if present.
        # files/ at the build root is baked into the firmware squashfs image.
        if [ -d "boards/${BOARD}/files" ]; then
            echo "Copying board files overlay from boards/${BOARD}/files/ ..."
            cp -rT "boards/${BOARD}/files" files/
        fi
